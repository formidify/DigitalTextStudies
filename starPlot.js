...
d3.csv('tweets_stars.csv', function(csvData) {
	...
	function calc_scale_cos(d) {
		return +d['x'] - 10000*(+d['prop'])*Math.cos(0.01745329252*(90*(+d['score']) + 90));
	}

	function calc_scale_sin(d) {
		return +d['y'] + 10000*(+d['prop'])*Math.sin(0.01745329252*(90*(+d['score']) + 90));
	}
...
svg.select(".legendSequential")
  .call(legendSequential);
	...
	var line = svg.selectAll('.starLine').data(csvData)
	...
	line.enter()
    	...
    	.style('stroke', function(d) {return colorScale(+(d['expected'])); })
   ...
   function mouseout(d, i) {
        ...
        d3.select(this).style("stroke", function(d) { return colorScale(+(d['expected_scaled'])); })
                       .style("stroke-opacity", 0.2)
                       .attr("stroke-width", 1);
